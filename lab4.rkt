  ...
  (make-set
   (cond
     ((or
       (null? LA) (null? LB)) '() )
      ((element? (car LA) LB)
       (set-difference (cdr LA) LB))
      (else
       (cons (car LA) (set-difference (cdr LA) LB))))))
...
(define (disjoint? SA SB)
  (cond
    ((or
      (null? SA) (null? SB)) #t)
    (else
     (equal? (sym-diff SA SB) (union SA SB)))))
...
(define (insert e L)
  (cond((element? e L)(make-set L))
       (else(make-set(append (list e) L)))))
