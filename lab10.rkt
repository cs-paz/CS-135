...
(define (get-vertices G)
 (usort (cond
          ((equal? G '()) '())
        (else
         (cons (caar G) (cons (cadar G) (get-vertices (cdr G))))))
               ))
...
(define (degree G v)
  (cond ((equal? G '()) 0)
        (else
         (+
          (+
           (if (= v (caar G)) 1 0) (if (= v (cadar G)) 1 0)) (degree (cdr G) v)))
               ))
...
(define (all-even? G)
  (define  (allEvenHelp a)
    (cond ((equal? a '()) #t)
          ((not(even? (degree G (car a)))) #f)
          (else (allEvenHelp (cdr a)))
             ))
  (allEvenHelp (get-vertices G)))
...
(define (adjacent G v)
  (usort
   (cond
        ((equal? G '()) '())
        ((equal? v (caar G))
          (cons (cadar G) (adjacent (cdr G) v)))
        ((equal? v (cadar G))
          (cons (caar G) (adjacent (cdr G) v)))
...
(define (connected? G)
  (if (null? G) #t
      (null?  (remove (get-vertices G) (usort (dfs G '() (caar G)))))
          ))
