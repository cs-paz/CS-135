...
(define (pred p)
  (if (equal? p '()) (eopl:error "pred error: zero has no predecessor!")
      (car p)))
...
(define (p- a b)
  (cond
    ((and (pz? a) (pz? b)) (eopl:error "subtraction error: a < b!"))
        (pz? b)
...
(define (p% a b)
  (cond ((p= a b) (p- a b))
        ((pz? b) (eopl:error "modulo error: mod is zero!")) 
        ((p> b a) a)
        (else (p% (p- a b) b))
              ))
...
;;   (tst1 p-even? 25) -> #f
;;   (tst1 p-even? 76) -> #t
;;(or (pz? p) (pz? (pred (pred p))))
;; Type signature: (p-even? peano) -> bool
(define (p-even? p)
  (cond ((pz? p) #t)
        ((pz? (pred p)) #f)
        ((pz? (pred (pred p))) #t)
        (else (pz? (p% p (p- p (pred (pred p))))))
               ))
...
(define (p-prime? p)
  (cond
    ((or
          (pz? (pred p)) (pz? p)) #f)
        (else (prime-help p (pred p))
              )))

(define (prime-help p predp)
  (cond
    ((pz? (pred predp)) #t)
        ((pz? (p% p predp)) #f)
        (else (prime-help p (pred predp)))
              ))
