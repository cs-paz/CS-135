...
;; Pledge: I pledge my honor that I have abided by Stevens honor code.
;;-------------------------------------------------------------------------------
...
(define (compose RO RI)
  (if (or (null? RI) (null? RO))
      '()
  (union (if (equal? (car (cdr (car RI))) (car(car RO)))
  (list (list (car(car RI)) (car (cdr (car RO)))))
  '())
  (union (compose (cdr RO) RI) (compose RO (cdr RI))))))
...
(define (power R k)
  (define (helper prev current k)
    (if (equal? k 0) '
        ()
    (if(equal? k 1)
       current
       (helper prev (compose prev current) (- k 1))))) (helper R R k))
...
(define (EQ-relation? R n)
  (and
   (symmetric? R) (reflexive? R n) (transitive? R)))
