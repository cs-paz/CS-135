...
(define (CRT-exists? cong-sys)
    (cond ((equal? cong-sys '()) #t)
        ((equal? (cdr cong-sys) '()) #t)
        ((= (euclid-gcd (cadar cong-sys) (cadar(cdr cong-sys))) 1)
         (and
           (CRT-exists? (cons (car cong-sys) (cddr cong-sys)))
...
(define (m cong-sys)
  (cond ((equal? cong-sys '()) 1)
        (else
         (* (cadar cong-sys) (m (cdr cong-sys))))
        ))
...
(define (CRT-helper cong-sys m)
  (if (CRT-exists? cong-sys)
       (cond ((equal? cong-sys '()) 0)
        (else (+ (* (*(caar cong-sys)  (quotient m (cadar cong-sys)))
                    (mul-inv (quotient m (cadar cong-sys))  (cadar cong-sys)))
...
(define (CRT cong-sys)
  (cond ((not (CRT-exists? cong-sys)) -1)
        (else
